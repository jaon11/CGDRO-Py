...
class UtilModels:
    ...
    def fit_f(self, X, y):
        """Fit a probability/response model on the source domain."""
        ...
        if self.mode == 'cls':
            ...
            if self.f_learner == 'linear':
                estimator = LogisticRegression(solver='lbfgs', max_iter=10000, random_state=self.seed)
                param_grid = None
            ...
            elif self.f_learner == 'mlp':
                # 3 hidden layers; keep modest sizes for stability
                ...
                param_grid = {
                    ...
                    'learning_rate_init': [1e-3, 5e-4],     # step size
                }

            else:
                assert False, "Unknown f_learner for classification"
            ...
            if self.f_learner == 'linear':
                ...
                if self.lambda_val is None or self.lambda_val == "CV.min":
                    estimator = LassoCV(cv=10, fit_intercept=self.intercept, max_iter=max_iter, random_state=self.seed)

                elif self.lambda_val == "CV":
                    est_cv = LassoCV(cv=10, fit_intercept=self.intercept, max_iter=max_iter, random_state=self.seed).fit(X, y)
                    mse_path_mean = est_cv.mse_path_.mean(axis=1)
                    mse_path_std = est_cv.mse_path_.std(axis=1)
                    ...
                    alpha_1se = est_cv.alphas_[idx_1se]
                    estimator = Lasso(alpha=alpha_1se, fit_intercept=self.intercept, max_iter=max_iter, random_state=self.seed)

                else:
                    estimator = Lasso(alpha=self.lambda_val, fit_intercept=self.intercept, max_iter=max_iter, random_state=self.seed)
                ...
                assert False, "Unknown f_learner for regression"
        ...
        if self.split:
            ...
            if param_grid is None:
                ...
                if self.mode == "cls":
                    ...
                    gridA = GridSearchCV(estimator=estimator, param_grid=param_grid, cv=stratified_kfold,
                                        scoring='neg_log_loss', verbose=verbose, n_jobs=-1)
                    gridB = GridSearchCV(estimator=estimator, param_grid=param_grid, cv=stratified_kfold,
                                        scoring='neg_log_loss', verbose=verbose, n_jobs=-1)
                elif self.mode == "reg":
                    kfold = KFold(n_splits=5, shuffle=True, random_state=self.seed)
                    gridA = GridSearchCV(estimator=estimator, param_grid=param_grid, cv=kfold,
                                        scoring='neg_mean_squared_error', verbose=verbose, n_jobs=-1)
                    gridB = GridSearchCV(estimator=estimator, param_grid=param_grid, cv=kfold,
                                        scoring='neg_mean_squared_error', verbose=verbose, n_jobs=-1)
                ...
                self.proba_params = gridA.best_params_
            ...
            if param_grid is None:
                ...
                if self.mode == "cls":
                    stratified_kfold = StratifiedKFold(n_splits=5, shuffle=True, random_state=self.seed)
                    grid = GridSearchCV(estimator=estimator, param_grid=param_grid, cv=stratified_kfold,
                                        scoring='neg_log_loss', verbose=verbose, n_jobs=-1)
                elif self.mode == "reg":
                    kfold = KFold(n_splits=5, shuffle=True, random_state=self.seed)
    ...
    def pred_f(self, X, X0):
        ...
        if self.mode == 'cls':
            if self.split:
                ...
                predAB = modelA.predict_proba(XB)  # P_A on XB
                predBA = modelB.predict_proba(XA)  # P_B on XA
                predX0 = 0.5 * modelA.predict_proba(X0) + 0.5 * modelB.predict_proba(X0)

                predX = np.zeros((X.shape[0], predAB.shape[1]))
            ...
            # Clip to avoid numerical issues downstream
